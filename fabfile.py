...
def cria_links():
	if env.environment == 'staging':
		...
		sudo('ln -sf {}/deploy/production/supervisor.conf /etc/supervisor/conf.d/mscmc.conf'.format(PROJECT_ROOT))
		sudo('ln -sf {}/deploy/production/nginx.conf /etc/nginx/sites-enabled/mscmc'.format(PROJECT_ROOT))
		sudo('chmod a+x {}/deploy/production/run.sh'.format(PROJECT_ROOT))
	else:
		print('Nenhum ambiente selecionado. Defina staging ou production.')
...
def restart_nginx_supervisor():
	...
	sudo('supervisorctl reread')
	sudo('supervisorctl restart mscmc')
